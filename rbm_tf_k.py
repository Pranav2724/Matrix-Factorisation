...
def convert_probs_to_ratings(probs):
    ...
    # return out
    return probs.dot(one_to_ten) / 2  # We divide by 2 to scale it from 1 tp 10 to 0 to 5
    ...
    def __init__(self, D, M, K):
        ...
        def build(self, D, M, K):
            ...
            objective = tf.reduce_mean(
                input_tensor=self.free_energy(self.X_in)) - tf.reduce_mean(
        ...
        def fit(self, X, mask, X_test, mask_test, epochs=10, batch_sz=256,
                show_fig=True):
            N, D = X.shape
            ...
            for i in range(epochs):
                t0 = datetime.now()
                print("epoch:", i)
                X, mask, X_test, mask_test = shuffle(X, mask, X_test,
                                                     mask_test)  # everything has to be shuffled accordingly
